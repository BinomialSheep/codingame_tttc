...
class Node {
 public:
  State state;
  list<shared_ptr<Node>> child_nodes;
  ...
  void expand() {
    ...
    for (const auto action : legal_actions) {
      child_nodes.push_back(make_shared<Node>(state));
      (*child_nodes.back()).state.advance(action);
    }
  }

  // どのノードを評価するか選択する
  shared_ptr<Node>& nextChildNode() {
    ...
    list<shared_ptr<montecarlo::Node>>::iterator best_it = child_nodes.end();
    ...
    for (auto it = child_nodes.begin(); it != child_nodes.end();) {
      ...
      double ucb1_value =
          1. - (*child_node).win_count / (double)(*child_node).visit_num +
...
pair<actionType, double> exec_mcts(Node& root_node,
  ...
  int draw_idx = -1;
  int lose_count = 0;
  list<shared_ptr<montecarlo::Node>>::iterator best_it =
  ...
  auto find_action_iterator =
      [&](actionType action) -> list<shared_ptr<Node>>::iterator {
