...
/*
転置テーブルを入れたけど、バグってるのか入れ方が悪いのか全く勝てないので没
*/
...
// array<unordered_map<uint64_t, Node*>, pow_4_9> zobrist_node_map;
array<array<unordered_map<uint64_t, float*>, 2>, pow_4_9> zobrist_win_count_map;
array<array<unordered_map<uint64_t, int*>, 2>, pow_4_9> zobrist_visit_num_map;
...
class ZobristHash {
 ...
 public:
  ...
  static uint64_t compute_hash_1(array<int, 9>& board_int, int big_board_int) {
    uint64_t ret = 0;
    rep(i, 9) {
  ...
  // static uint64_t compute_hash_2(array<int, 9>& board_int, int big_board_int)
  // {
  //   uint64_t ret = 0;
  //   rep(i, 9) {
  //     if (big_board_int & (3 << i)) continue;
...
class Node {
 public:
  ...
  int action_count;
  uint64_t hash;
  float* shered_win_count;
  int* shered_visit_num;
  ...
  Node(State& arg_state)
      : state(arg_state),
        ...
        action_count(0),
        hash(
            ZobristHash::compute_hash_1(state.board_int, state.big_board_int)) {
    if (zobrist_win_count_map[state.big_board_int][state.is_x].count(hash)) {
      shered_win_count =
          zobrist_win_count_map[state.big_board_int][state.is_x][hash];
      shered_visit_num =
          zobrist_visit_num_map[state.big_board_int][state.is_x][hash];
    } else {
      shered_win_count = new float;
      *shered_win_count = 0;
      zobrist_win_count_map[state.big_board_int][state.is_x][hash] =
          shered_win_count;
      shered_visit_num = new int;
      *shered_visit_num = 0;
      zobrist_visit_num_map[state.big_board_int][state.is_x][hash] =
          shered_visit_num;
    }
  ...
  Node& nextChildNode() {
    ...
    rep(i, child_nodes.size()) {
      ...
      double ucb1_value = 1. -
                          *child_node.shered_win_count /
                              (double)(*child_node.shered_visit_num) +
